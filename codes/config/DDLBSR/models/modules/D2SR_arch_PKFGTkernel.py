...
class DDLNet(nn.Module):
    ...
    def __init__(self, nf=64, ksize=21, num_crblocks=5,conv_k_sz=3):
        super().__init__()
        ...
        self.conv_tail=nn.Conv2d(nf, ksize**2, 3,1,1)
        self.conv_tail2 = nn.Conv2d(ksize**2,ksize**2,3,1,1)
...
class D2SR_s2(nn.Module):
    def __init__(self, nf=64, ksize=21, scale=4):
        super().__init__()
        self.kernel_size = ksize
        self.scale = scale
        self.dpnet = Encoder()
        
        self.ddlnet = DDLNet(nf=nf, ksize=ksize)
...
class D2SR_s3(nn.Module):
    ...
    def __init__(self, nf=64, scale=4, ksize=21, n_resblock=3):
        ...
        # stage2 退化核预测网络
        # self.ddlnet = DDLNet(ksize=ksize)
        # stage3 重构网络
        self.c_conv = CorrectKernelBlock(self.kernel_size**2, nf, 3)
    ...
    def forward(self, x, real_K=None):
        ...
        kernel_for = real_K.flatten(1).unsqueeze(-1).unsqueeze(-1)
        kernel_for = kernel_for.repeat(1, 1, x.size(-2), x.size(-1))
        # with torch.no_grad():
        d_predic = self.dpnet(x)

        # kernel prediction
        # kernel_for = self.ddlnet(x)
        # print('2', kernel_for.shape)
...
if __name__ == '__main__':
    model = D2SR_s3(scale=2, ksize=11)

    
    print(model)

    x = torch.randn((2, 3, 64, 64))
    y = torch.randn((2,11,11))
    x = model(x, y)
