...
class B_Model(BaseModel):
    def __init__(self, opt, netg2=None):
        ...
        #     print(self.netG)
        #     print('model size:{:.5f}M'.format(sum(p.numel() for p in self.netG.parameters()) / 1000000.0))
        #     zzzz
        if netg2 is not None:
            self.netG2 = netg2
        if opt["dist"]:
    ...
    def optimize_parameters(self, step):
        ...
        # sr, kernels, kernel, diff  = self.netG(self.var_L, self.real_kernel)
        sr, diff  = self.netG(self.var_L, self.real_kernel)
